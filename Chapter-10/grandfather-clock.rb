...
def grandfather_clock(block, mapping)
  current_time = 0..Time.now.hour

  current_time.each do |hour|
    block.call(mapping, hour)
  end
end

clock = Proc.new do |k, v|
  puts "DONG! it's #{k[v]}"
end

grandfather_clock(clock, time_mapping)
